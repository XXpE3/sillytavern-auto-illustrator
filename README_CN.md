...
- 🔀 **独立 API 调用模式** (v1.5.0+)：实验性提示生成模式
  ...
  - 通过每条消息最大提示数量限制来控制成本
  - 自动聊天历史修剪
  - 可选的 LLM 模型 ID 可让 Auto Illustrator 在当前聊天补全或 OpenAI 兼容连接上，用另一个模型处理独立提示词生成调用
...
- **组件可见性** (v1.3.0+)：
  ...
  - **注意**：更改需要重新加载页面才能生效
- **重置为默认值**：恢复所有设置为默认值
- **Auto Illustrator 的 LLM 模型 ID**：可选模型 ID，用于 Auto Illustrator 自己发起的独立 LLM 调用。留空则使用 SillyTavern 当前模型。填写后只影响独立 API 提示词生成和更新提示词，并要求当前连接为聊天补全或 OpenAI 兼容连接。
...
- 如果设置了 **Auto Illustrator 的 LLM 模型 ID**，该更新提示词调用会使用指定模型；留空则使用 SillyTavern 当前模型
